 ...
 *
 * The Knowledge department contributes via its progress value (set by the
 * host when AI terms are solved), so a fully-solved knowledge deck also
 * gives 100% to the average.
 */
export function calculateFactoryHealth(state: FactoryStateData): number {
  const { power, data, security, model, knowledge } = state;
